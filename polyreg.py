...
def train_polyreg():
	...
	data_dir = os.path.join(BASE_DIR, 'Poly_data')
...
def test_polyreg(lin_reg):
	t = time()

	pred = lin_reg.predict(poly_reg.fit_transform(X_train))
	...
	output = lin_reg.predict(poly_reg.fit_transform(X_test))
	
	print("The running time: ",time()-t)

def predict_polyreg(lin_reg, inp):
	t = time()
	#inp = sc.transform(inp)
	medianreg = np.exp(y_train).median()
	output = lin_reg.predict(poly_reg.fit_transform(inp))
